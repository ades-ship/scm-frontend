...
import { DeleteIcon, Edit2Icon, Expand, HeartIcon } from 'lucide-react';
import { Link } from "react-router-dom"
import axios from 'axios';

const ListView = ({contacts, getContacts}) => {
  ...
  return (
    <div>
      {/* show contact list */}
      {
        contacts ? <div className=''>
            <div className='flex justify-between items-center gap-2'>
              <h6 className='bg-neutral-100 p-2 font-medium w-full'>User Image</h6>
            ...
            <div className='flex flex-col gap-2 mt-10'>
            {
              contacts.map(contact=>(
                ...
                <div key={contact.id} className='flex justify-between items-center hover:bg-neutral-100 p-2 transition-all duration-500'>
        ...
        </div>: <div>To see contacts, first add contact!</div>
      }
    </div>
  )
}
